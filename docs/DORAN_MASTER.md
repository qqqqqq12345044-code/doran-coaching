...
이 문서는 **현재 코드 상태의 스냅샷**이다. 작성 기준일: 2026-09-11 (로컬 커밋 `d49c3a8`
완료, push 대기 — 상세는 HISTORY 최신 항목 참고). 이 문서와 실제 코드가 다르면 항상
**코드가 우선**한다 — 큰 작업 완료 후 이 문서를 갱신하되, 갱신을 놓친 부분이 있을 수
있음을 전제하고 의심되면 코드를 다시 읽는다.
