...
// #consultation, #how-it-works)로만 연결한다.
const HERO_SLIDES: HeroSlide[] = [
  ...
  },
  {
    ...
    secondaryCta: { label: "무료 상담 신청", href: "#consultation" },
  ...
  },
  {
    ...
    secondaryCta: { label: "상담 신청", href: "#consultation" },
...
// 메인 홈 Hero. 배경 이미지 + 핵심 메시지(eyebrow/H1/설명/CTA)가 슬라이드
// 단위로 함께 바뀐다. 이미지는 기존처럼 부드럽게 crossfade되고, 텍스트는
// key 변경으로 매번 새로 mount되어 hero-item/hero-image-in keyframe이
// 슬라이드마다 다시 재생된다(페이지 최초 진입 시에도 동일한 방식으로 한 번
// 재생됨 — 별도 분기 불필요). prefers-reduced-motion에서는 자동 전환을
// 하지 않는다.
    ...
    <section className="relative flex min-h-[540px] w-full items-center overflow-hidden sm:min-h-[600px] lg:min-h-[680px]">
      ...
      <div className="section-shell relative py-20 sm:py-24">
        <div className="max-w-xl">
          {/* key가 슬라이드마다 바뀌어 이 블록들이 매번 새로 mount되고,
              tailwind.config.ts의 hero-item/hero-image-in keyframe이 처음부터
              다시 재생된다(레이스 컨디션 없는 순수 CSS animation). */}
          ...
          <h1
            key={`h1-${activeIndex}`}
            className="text-balance animate-hero-item mt-5 text-[32px] font-extrabold leading-[1.2] tracking-tight text-white sm:text-[44px] md:text-[52px]"
