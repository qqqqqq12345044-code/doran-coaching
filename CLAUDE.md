...
git 안전, 파일 우선 확인, 검증 원칙, 효율/토큰, 커뮤니케이션 등 프로젝트 공통 행동 규칙은
`~/.claude/CLAUDE.md`(전역)를 따른다. 이 문서는 **DORAN 고유 규칙만** 다룬다.
...
(파일 우선 확인, 범위 밖 수정 금지, 원인 우선 진단, 불필요한 전체 재탐색 금지는 전역 규칙 참고)

- "더 좋게 해줘" → 시각적 화려함이 아니라 실용성/가독성/전환율/유지보수성을 함께 고려한다.

## [작업 효율 / 토큰 최적화]

Claude Code 사용량/토큰 절약 습관. 마지막 항목(예외)이 항상 우선한다.

- 파일 1~5개 탐색/수정·grep/rg·git 상태 확인·단순 버그/문구/metadata 수정·단일 페이지 검수는
  직접 처리한다. Subagent는 서로 독립적인 작업이 2개 이상이고 병렬화로 시간이 크게 줄 때만 사용.
- HSK/JLPT/TOEIC 등 특정 언어·자격증·SEO 페이지 작업은 해당 data/공통 template/직접 영향받는
  component만 먼저 확인하고, 관련 없는 페이지는 재조회하지 않는다.
- 검증은 관련 범위 확인 → lint/typecheck → build 순으로 점진적으로 수행한다. 전체
  crawl/Playwright는 공통 구조 변경이나 배포 전 체크포인트에서만 수행한다.
- metadata/canonical/RSS/sitemap/robots처럼 UI 변화가 없는 수정은 스크린샷 검수를 생략하고
  validator + 실제 출력만 확인한다. 기존 validator가 이미 검증하는 항목은 수동으로 재검사하지 않는다.
- Vercel 배포는 push 후 1회 확인하고, 실패했을 때만 상세 로그를 본다.
- 성공 로그/raw output은 최소화하고 실패한 항목만 상세 분석한다. Git 기록과 기존 QA 문서를 작업
  기억장치로 활용하고, 이미 commit된 완료 작업은 다시 조사하지 않는다.
- 완전히 다른 작업으로 넘어갈 때는 `/clear`, 같은 작업을 계속 이어갈 때는 `/compact`를 권장한다.
- **예외(정확성 우선)**: sitemap/RSS 구조, noindex/canonical, production 배포, git history, 데이터
  삭제, 보안/개인정보/API credential, 공통 routing/template 변경은 위 규칙으로 검증을 생략하지 않는다.
...
- 한국어 응답 중 코드/API/고유명사만 필요 시 영어, 명시적 요청 없이 일본어 사용 금지(전역: 한국어 응답 원칙).
- 초보자도 이해 가능하게 쓴다(간결함 자체는 전역 규칙 참고).
